...
/// What a generated file owes its reader: that it is generated, and that
/// editing it is wasted work. What it *contains* is answered by reading it,
/// which is nearer to hand than any paragraph about it.
const _header = '''
...
/// scene's formal can default to it, and another set — a dark palette, a
/// denser ramp — is the same class with other arguments, built by the app
/// and passed at the call. That constructor is the whole override story:
/// the library declares one set and the editor draws it.
///
...
/// One field of a style, spelled the way the SCENE FILE spells it — through
/// the property table, so the two emitters cannot drift.
///
/// They had drifted, and silently: this was a switch over the value's type
/// whose last arm fell through to `'$value'`, a `toString()`. Enums survived
/// by luck, because a Dart enum prints as its own source. A paint stack did
/// not: a list of layers printed as `[StrokeLayer(21.9, SolidPaint(Color(…)))]`
/// — positional where the constructors are named, and `Color` where the type
/// is `SceneColor` — so a library holding a style with any paint pass
/// generated a `scene_args.dart` that did not compile, and the app it was
/// generated for stopped building.
String _styleField(String field, Object value) {
  for (var p in sceneStyleProps) {
    if (p.name == field) return scenePropLiteral(p, value);
  }
  // Not a property of the table: it cannot have got into a style, and
  // guessing at its source is what caused the bug above.
  throw ArgumentError('a style has no "$field" to spell');
}
...
/// A token's value as a const literal.
///
/// Every arm is a type the four token kinds and a style can actually hold.
/// There is no fall-through: a `toString()` is not source, and the one that
/// used to sit here shipped a file that did not compile.
String _literal(Object value) => switch (value) {
  ...
  bool b => '$b',
  var v => throw ArgumentError('a token cannot hold a ${v.runtimeType}'),
};
